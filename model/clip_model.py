...
class CLIPAttention(torch.nn.Module):
    def __init__(self, embed_dim, heads, dtype, device, operations, **kwargs):
        super().__init__()
        self.abs_configs = kwargs
...
ACTIVATIONS = {"quick_gelu": lambda a: a * torch.sigmoid(1.702 * a),
               ...
               "swi_glu": lambda a: a * torch.sigmoid(a),
               "relu": torch.nn.functional.relu,
               "quick_relu": lambda a: torch.nn.functional.relu(a, inplace=True),
               "silu": torch.nn.functional.silu,
               "mish": lambda a: a * torch.tanh(torch.nn.functional.softplus(a)),
               "swish": lambda a: a * torch.sigmoid(a),
               "tanh": torch.tanh,
               "identity": lambda a: a,
               ...
               "soft_tau": lambda a, tau=1.0: torch.nn.functional.softmax(a / tau, dim=-1),
               "softmax": torch.nn.functional.softmax,
               "tanhshrink": torch.nn.functional.tanhshrink,
               "top_k": lambda a, k=1: torch.topk(a, k=k, dim=-1).values,
               "top_p": lambda a, p=0.9: torch.nn.functional.softmax(a, dim=-1) * (torch.cumsum(torch.nn.functional.softmax(a, dim=-1), dim=-1) <= p).float()
